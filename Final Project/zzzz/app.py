...
try:
    ...
    from agents.autopulse_agent import monitor_inventory as autopulse_monitor
    AGENTS_ENABLED = True
...
# Register Agent Routes if available
if AGENTS_ENABLED:
    app.register_blueprint(agent_bp)
    print("✓ Agentic AI routes registered")
...
try:
    ...
    admins.create_index([("location", "2dsphere")])
...
def update_blood_inventory():
    ...
    try:
        ...
        if result.modified_count > 0:
            # Trigger AutoPulse Agent to check inventory
            if AGENTS_ENABLED:
                try:
                    autopulse_monitor.delay(admin_id)
                    logger.info(f"AutoPulse agent triggered for hospital {admin_id}")
                except Exception as e:
                    logger.warning(f"Failed to trigger AutoPulse agent: {str(e)}")
